...
#### 3.
##### A. FindOne
      -  해당 req.email값으로 User를 조회하고, 조회된 데이터를 user로 받는다.
      -  user가 존재한다면 comparePassword를 실행한다.
##### B. ComparePassword
      - req.password값과 user.password를 비교한 뒤, 동일한지 확인한다.
      - 동일하다면 generateToken을 실행한다.
##### C. GenerateToken
      - user에서 .id 값을 16진수로 변환한 뒤, secret값과 함께 jwt를 통해 인코딩을 하고 user.token으로 반환한다.
      - 토큰 유효기간을 설정한뒤, user.tokenExp로 받아온다.
      - user에 user.token과 user.tokenExp값을 저장한다.
      - 해당 값을 쿠키에 저장한다.
